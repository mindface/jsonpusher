	...
	return (
		<section className="section-level-up">
			...
			<div className="details">
				<p className="text pb-2">アドバイスに関しては、必ず検証してみることが普通でしょうか。</p>
				<p className="text pb-2">アドバイスが運動の癖を把握した情報量かも考えることになります。</p>
				<p className="text pb-4">実際に目的と検証過程から利用して、継続的にメリットがあれば採用することになりそうでしょうか。</p>
				...
				<div className="details-result p-4 pb-8">
					{selectAdviceList.map((adviceItem) => <div key={adviceItem.value} className="">
						<div>
							{ adviceItem.skillLevel < experiencePoints && <div className="pb-2">{adviceItem.label}</div>}
						</div>
					</div>)}
				</div>
				<div className="p-4 border rounded-lg">
					<Title3h title="アドバイスは経験があると1つのパタンになる" size="medium" />
					<p className="text pb-4">誰かからアドバイスされて、練習した経験があるのではないでしょうか。</p>
					<p className="text pb-4">経験を積むと情報が増えていくので、その1つにアドバイスされたことが含まれるようになります。</p>
					<p className="text pb-4">ケースや調子の応じて利用するかどうか選択することになるので、自分で評価を持つといいかもしれません。</p>
					<div className="pb-2">
						<Button
							label="計画を見直す"
